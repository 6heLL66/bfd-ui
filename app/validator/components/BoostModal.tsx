...
interface BoostModalProps {
  ...
  onQueueBoost: (amount: bigint) => Promise<void>;
}
...
export const BoostModal = ({ 
  ...
  const handleSetMaxAmount = () => {
    ...
    setBoostAmount(maxAmount?.toSignificant(18) ?? '');
  };

  const handleConfirm = async () => {
    const amount = TokenAmount.fromHumanAmount(bgtToken, boostAmount as `${number}`);
    await onQueueBoost(amount.amount);
    onClose();
  };
  ...
  const isError = +boostAmount > Number(availableBGT);
            ...
            <div className="bg-surface backdrop-blur-xl rounded-2xl border-2 border-border/40 shadow-2xl shadow-primary-default/10 p-6 w-full max-w-md max-h-[90vh] overflow-y-auto"
              ...
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center">
                  ...
                  <h3 className="text-xl font-bold">Add Boost</h3>
                </div>
                <button 
              ...
              <div className="mb-4 flex justify-between items-center bg-primary-default/10 rounded-lg px-4 py-3 border border-primary-default/20">
                <span className="text-sm font-medium text-foreground-secondary">Available BGT</span>
