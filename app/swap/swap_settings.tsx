...
export const SwapSettings = ({
  ...
  onSlipageChange: (slippage: string) => void;
  ...
  return (
    <Popover>
      <PopoverTrigger>
        <button className="p-2 rounded-lg hover:bg-surface/80 transition-colors">
      ...
      <PopoverContent className="bg-surface border-2 border-border/40 rounded-xl p-4 shadow-xl max-w-[400px]">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-sm text-foreground-secondary">Slippage Tolerance</label>
              ...
              {slippageOptions.map(option => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                    slippage === option ? 'bg-primary-default/20 text-primary-default border border-primary-default/40' : 'bg-surface hover:bg-surface/80 border border-border/40'
                  }`}
                >
                  {option}%
                </button>
          ...
          <div className="space-y-2">
            <label className="text-sm text-foreground-secondary">Transaction Deadline</label>
          ...
          <div className="text-xs text-foreground-secondary mt-4">Your transaction will revert if the price changes unfavorably by more than this percentage.</div>
        </div>
      </PopoverContent>
    </Popover>
  );
