...
const SocialButton = ({
  ...
  return (
    <Link
      href={href}
      target="_blank"
      className={`group flex items-center text-body font-bold gap-3 rounded-lg p-2 transition-all hover:bg-white/5 ${className}`}
      style={{ color }}
      rel="noopener noreferrer"
    >
      <div className="rounded-md border-2 p-2 transition-transform group-hover:scale-110" style={{ borderColor: color, color }}>
        {icon}
      </div>
      <span className="font-medium transition-colors group-hover:text-white" style={{ color }}>
        {title}
      </span>
    </Link>
  );
...
const DiscordIcon = () => {
  return (
    ...
    >
      ...
      <ellipse cx="30.5" cy="26" fill="currentColor" rx="3.5" ry="4"></ellipse>
      <ellipse cx="17.5" cy="26" opacity=".05" rx="4.5" ry="5"></ellipse>
      <ellipse cx="17.5" cy="26" opacity=".05" rx="4" ry="4.5"></ellipse>
      <ellipse cx="17.5" cy="26" fill="currentColor" rx="3.5" ry="4"></ellipse>
    </svg>
  );
};
